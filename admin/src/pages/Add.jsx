...
const uploadImageToCloudinary = async (file) => {
  const signatureRes = await axios.get(`${backendUrl}/api/cloudinary/signature`,{ withCredentials: false });
  const { signature, timestamp, api_key, cloud_name } = signatureRes.data;

  const imgForm = new FormData();
  imgForm.append("file", file);
  imgForm.append("api_key", api_key);
  imgForm.append("timestamp", timestamp);
  imgForm.append("signature", signature);
  ...
  try {
    ...
    const imageUrls = [];
    if (image1) imageUrls.push(await uploadImageToCloudinary(image1));
    if (image2) imageUrls.push(await uploadImageToCloudinary(image2));
    if (image3) imageUrls.push(await uploadImageToCloudinary(image3));
    if (image4) imageUrls.push(await uploadImageToCloudinary(image4));

    formData.append("images", JSON.stringify(imageUrls)); // 👈 Final image array
...
const uploadImageToCloudinary = async (file) => {
  try {
    ...
    const signatureRes = await axios.get(`${backendUrl}/api/cloudinary/signature`, {
      headers: {
        'Content-Type': 'application/json',
        ...(token && { token }) // Add token if available
      },
  ...
  try {
    ...
    // Add proper headers including token
...
const Add = ({token}) => {
  ...
  try {
    ...
    // Add image files directly to formData (multer will handle these)
    ...
    const response = await axios.post(`${backendUrl}/api/product/add`, formData, {
      headers: {
        token: localStorage.getItem("token") // must match backend header check
    }
    });
